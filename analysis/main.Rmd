...
# compare saudi-arabien, vereinigte emirate, deutschland, usa
Kategorie_LandJahr <- export_restructured  %>% 
  filter(Land %in% c("USA", "Deutschland", "Vereinigte Arabische Emirate", "Saudi-Arabien")) %>%
   group_by(variable, Land) %>%
   summarize(value = sum(value)) %>% # Exportvolumensummen pro Kategorie berechnen
   arrange(desc(value)) # der Größe nach sortieren
